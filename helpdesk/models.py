...
class DepartmentManager(KangarooModel):
    ...
    objects = KangarooCompanyManager("manager__employee_work_info__company_id")
...
class TicketType(KangarooModel):
...
class ClaimRequest(KangarooModel):
...
class Attachment(KangarooModel):
...
class FAQCategory(KangarooModel):
