...
class FilingStatus(KangarooModel):
    """
    FilingStatus model
    """
...
class LoanAccount(KangarooModel):
    """
...
class Reimbursement(KangarooModel):
    """
    Reimbursement Model
    """
            ...
            if self.status == "approved" and self.allowance_id is None:
                if self.type == "reimbursement":
                    ...
                    if bonus_points.points >= self.bonus_to_encash:
                        ...
                        request = getattr(
                            kangaroo_middlewares._thread_locals, "request", None
                        )
                        if request:
                            messages.info(
                                request,
...
class ReimbursementrequestComment(KangarooModel):
    """
