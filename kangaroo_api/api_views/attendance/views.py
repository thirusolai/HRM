        ...
        if not request.user.employee_get.check_online():
            ...
            if employee and work_info is not None:
                ...
                if start_time_sec > end_time_sec:
                    # night shift
                    # ------------------
                    # Night shift in Kangaroo consider a 24 hours from noon to next day noon,
...
class MailTemplateView(APIView):
    ...
    def get(self, request):
        instances = KangarooMailTemplate.objects.all()
...
class ConvertedMailTemplateConvert(APIView):
    ...
    def put(self, request):
        ...
        bdy = KangarooMailTemplate.objects.filter(id=template_id).first()
