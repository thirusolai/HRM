...
from base.kangaroo_company_manager import KangarooCompanyManager
from kangaroo import kangaroo_middlewares
from kangaroo.kangaroo_middlewares import _thread_locals
from kangaroo.models import KangarooModel, upload_path
from kangaroo_audit.models import KangarooAuditInfo, KangarooAuditLog
...
class Department(KangarooModel):
    """
    Department model
    """
    ...
    objects = KangarooCompanyManager()
...
class JobPosition(KangarooModel):
    """
    JobPosition model
    """
    ...
    objects = KangarooCompanyManager("department_id__company_id")
...
class JobRole(KangarooModel):
    ...
    objects = KangarooCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(KangarooModel):
    """
    ...
    objects = KangarooCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(KangarooModel):
    """
...
class EmployeeType(KangarooModel):
    """
    EmployeeType model
    """
...
class EmployeeShift(KangarooModel):
    """
    EmployeeShift model
    """
    ...
    objects = KangarooCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(KangarooModel):
    """
    ...
    auto_punch_out_time = models.TimeField(
        ...
        help_text=_(
            "Time at which the kangaroo will automatically check out the employee attendance if they forget."
        ),
    )
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = KangarooCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShift(KangarooModel):
    """
    RotatingShift model
    """
...
class RotatingShiftAssign(KangarooModel):
    """
...
class WorkTypeRequest(KangarooModel):
    """
    ...
    def clean(self):
        request = getattr(kangaroo_middlewares._thread_locals, "request", None)
...
class WorkTypeRequestComment(KangarooModel):
    """
...
class ShiftRequest(KangarooModel):
    """
    ShiftRequest model
    """
...
class ShiftRequestComment(KangarooModel):
    """
...
class Tags(KangarooModel):
    title = models.CharField(max_length=30)
    color = models.CharField(max_length=30)
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT
    )
    objects = KangarooCompanyManager(related_company_field="company_id")
...
class KangarooMailTemplate(KangarooModel):
...
class DynamicEmailConfiguration(KangarooModel):
    """
...
class MultipleApprovalCondition(KangarooModel):
...
class Announcement(KangarooModel):
    """
...
class AnnouncementComment(KangarooModel):
    """
...
class DashboardEmployeeCharts(KangarooModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(KangarooModel):
...
class CompanyLeaves(KangarooModel):
...
class PenaltyAccounts(KangarooModel):
    """
