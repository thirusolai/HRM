...
from kangaroo import kangaroo_apps
from kangaroo.decorators import (
    delete_permission,
...
from kangaroo.kangaroo_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from kangaroo.methods import get_kangaroo_model_class, remove_dynamic_url
from kangaroo_audit.forms import HistoryTrackingFieldsForm
from kangaroo_audit.models import AccountBlockUnblock, AuditTag, HistoryTrackingFields
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        ...
        return render(request, "initialize_database/kangaroo_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/kangaroo_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/kangaroo_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/kangaroo_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/kangaroo_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/kangaroo_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/kangaroo_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/kangaroo_job_position.html",
...
class KangarooPasswordResetView(PasswordResetView):
    """
    Kangaroo View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    Kangaroo View for Employee Reset Password
    """
...
def two_factor_auth(request):
    ...
    if not kangaroo_apps.TWO_FACTORS_AUTHENTICATION:
        return redirect("/")
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_kangaroo_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    white_labelling = getattr(kangaroo_apps, "WHITE_LABELLING", False)
    image_path = path.join(settings.STATIC_ROOT, "images/ui/kangaroo-logo.png")
    company_name = "Kangaroo"
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from Kangaroo")
...
@login_required
@hx_request_required
@permission_required("base.change_kangaroomailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = KangarooMailTemplate.objects.get(id=obj_id)
...
@login_required
@hx_request_required
@permission_required("base.add_kangaroomailtemplate")
...
@login_required
@permission_required("base.delete_kangaroomailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = KangarooMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_kangaroo_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_kangaroo_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_kangaroo_model_class(
...
@login_required
@permission_required("kangaroo_audit.change_accountblockunblock")
...
@login_required
@permission_required("kangaroo_audit.view_audittag")
...
@login_required
@hx_request_required
@permission_required("kangaroo_audit.add_audittag")
...
@login_required
@hx_request_required
@permission_required("kangaroo_audit.change_audittag")
...
@login_required
def get_kangaroo_installed_apps(request):
